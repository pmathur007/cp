...
int search(vector<int> &p, string &str, string &s_i)
{
    int n = str.size(), sn = s_i.size();
    int l = 0, r = n - 1;
    while (l <= r)
    {
        int mid = (l + r) / 2;
        if (s_i == str.substr(p[mid], sn))
            return mid;
        else if (s_i.compare(str.substr(p[mid], sn)) < 0)
            r = mid - 1;
        else
            l = mid + 1;
    }
...
int main()
{
    ...
    for (int t = 0; t < T; t++)
    {
        string s_i;
        cin >> s_i;
        cout << (search(p, str, s_i) < 0 ? "No" : "Yes") << '\n';
    }
